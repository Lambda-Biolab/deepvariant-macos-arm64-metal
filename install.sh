...
echo "    --num_shards=\$(sysctl -n hw.perflevel0.logicalcpu)"
echo ""
if [[ "$(uname -m)" == "arm64" ]]; then
  echo "On Apple Silicon, run_deepvariant automatically uses:"
  echo "  - Metal GPU (tensorflow-metal) for 4.25x call_variants speedup (fallback when no CoreML model)"
  echo "  - CoreML replaces TF inference when model is present: Metal GPU + Neural Engine + CPU together (1.28x over Metal alone)"
  echo "  - Fast pipeline (concurrent make_examples + call_variants via shared"
  echo "    memory IPC) for ~1.9x total wall-time improvement"
  echo "  No extra flags needed — all acceleration is active by default."
  echo ""
fi
